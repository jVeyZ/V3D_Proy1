...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=f'Índice de la cámara (por defecto {config.CAMERA_INDEX})'
    )
    parser.add_argument(
...
class MiniGolfApp:
    ...
    def _init_camera(self):
        ...
        if self.use_demo:
            ...
            print(f"[Main] Intentando abrir cámara {self.args.camera}...")
            self.camera = cv2.VideoCapture(self.args.camera)
            if not self.camera.isOpened():
                print("[Main] ERROR: No se pudo abrir la cámara.")
                print("[Main] Sugerencia: usa --demo para modo simulado.")
                return False
...
def main():
    ...
    if not args.demo:
        test_cam = cv2.VideoCapture(args.camera)
